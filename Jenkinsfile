...
properties([
  parameters([
    ...
    string(defaultValue: 'android-8.0', description: 'GitHub repository', name: 'GITHUB_REPO'),
...
node('docker && android-build') {
  timestamps {
    wrap([$class: 'AnsiColorBuildWrapper', colorMapName: 'xterm']) {
      ...
      def environment = docker.build('build-environment:android-8.0', 'build-environment')
